...
Blunux의 푸른 펭귄도 그 계보 위에 있다.

로고 자체는 한국의 붓글씨 기법으로 그렸다. 단 **네 획**으로 펭귄의 형태를 잡고,
나머지는 채우지 않은 채 비워 둔다. 선으로 전부 그리지 않고 남긴 공간이 형태를 완성하는
한국 서예의 **여백(餘白)의 미**를 담은 것으로, 최소한의 것만 두고 나머지는 비워 둔다는
배포판의 방향과도 맞닿아 있다.
